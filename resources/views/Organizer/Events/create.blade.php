...
<form class="p-6 [#f7f8fc] shadow-xl max-w-screen-sm mx-auto mt-32 mb-10 rounded-3xl" action="{{ route('organizer.events.store') }}" method="POST">
    @csrf
    ...
    <div>
        ...
        <input type="date" name="date" value="{{old('date')}}" id="date" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 shadow" placeholder="Event date">
        @error('date')
            <span class="text-red-400">{{$message}}</span>
        @enderror
    </div>
